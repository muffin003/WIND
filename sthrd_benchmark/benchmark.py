...
class BenchmarkRunner:
    ...
    def get_dataframe(self):
        ...
        df = pd.DataFrame({
            ...
            'error': [np.linalg.norm(x - theta) for x, theta in zip(history['x'], history['theta'])]
        })
    ...
    def run_grid(self, param_grid, algorithm_class, n_trials=5, n_steps=500):
        ...
        for params in tqdm(param_combinations, desc="Parameter grid"):
            ...
            for trial in range(n_trials):
                ...
                algorithm = algorithm_class(dim=env.dim, **config.get('algorithm_params', {}))
            ...
            for metric_name in trial_results[0]['metrics'].keys():
                avg_metrics[metric_name] = np.mean([r['metrics'][metric_name] for r in trial_results])
