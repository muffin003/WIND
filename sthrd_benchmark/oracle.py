...
class ZeroOrderOracle(Oracle):
    def __init__(self, environment, perturbation_scale=0.01, 
                 value_noise=None, seed=None):
        super().__init__(environment, seed)
...
class HybridOracle(Oracle):
    def __init__(self, environment, grad_noise=None, 
                 value_noise=None, sparse_prob=0.1, seed=None):
        super().__init__(environment, seed)
        self.grad_noise = grad_noise
        self.value_noise = value_noise
    ...
    def query(self, x):
        if self.rng.rand() < self.sparse_prob:
            oracle = ZeroOrderOracle(self.env, perturbation_scale=0.01, 
                                   value_noise=self.value_noise, seed=int(self.rng.rand() * 1000))
            return oracle.query(x)
        else:
            oracle = FirstOrderOracle(self.env, grad_noise=self.grad_noise, 
                                    value_noise=self.value_noise, seed=int(self.rng.rand() * 1000))
            return oracle.query(x)