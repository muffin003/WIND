...
class BenchmarkVisualizer:
    ...
    def plot_trajectory_3d(self, max_points=500):
        ...
        if len(x_traj[0]) < 3:
            print(f"Trajectory has dimension {len(x_traj[0])}, need at least 3 for 3D plot")
            return
    ...
    def plot_comparison_radar(self, comparison_results):
        ...
        for algo_name in algorithm_names:
            values = [comparison_results[algo_name].get(metric, 0) for metric in metrics]
        ...
        max_val = max([max([v for v in comparison_results[algo].values() if v is not None]) 
    ...
    def plot_heatmap_analysis(self, grid_results_df, x_param, y_param, value_column='avg_error'):
        if grid_results_df.empty:
            return
        
        pivot = grid_results_df.pivot(index=y_param, columns=x_param, values=value_column)
